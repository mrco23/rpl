...
for (const [oldRel, newRel] of Object.entries(moveMap)) {
    ...
    if (!fs.existsSync(newAbs) || (!newAbs.endsWith('.js') && !newAbs.endsWith('.jsx'))) continue;

    let content = fs.readFileSync(newAbs, 'utf8');
    const currentDirOld = path.dirname(path.resolve(srcDir, oldRel));
    const currentDirNew = path.dirname(newAbs);

    let changed = false;

    content = content.replace(importRegex, (match, importStr) => {
    ...
    content = content.replace(dynamicImportRegex, (match, importStr) => {
        let absImportOld = resolveImportPath(currentDirOld, importStr);
        if (absImportOld) {
            let targetOldAbs = absImportOld;
            if (!oldToNew[targetOldAbs]) {
