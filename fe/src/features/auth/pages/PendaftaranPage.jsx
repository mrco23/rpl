...
export default function ApplicantRegisterPage() {
  ...
  const validateStep1 = () => {
    ...
    // A. Nama Lengkap
    if (!d.namaLengkap || d.namaLengkap.length < 3 || d.namaLengkap.length > 100 || !nameRegex.test(d.namaLengkap) || repeatRegex.test(d.namaLengkap.replace(/\s/g, ''))) {
      newErrors.namaLengkap = "Nama lengkap harus diisi dengan nama yang valid.";
    }
    ...
    // G. Asal Sekolah
    if (!d.asalSekolah || /(SMA|SMK|MA\b|MAN\b|Paket C)/i.test(d.asalSekolah) || !/(SD|MI|Sekolah Dasar)/i.test(d.asalSekolah)) {
    ...
    Object.keys(alamatFields).forEach(f => {
      if (!d[f] || d[f].length < 3 || !nameRegex.test(d[f]) || repeatRegex.test(d[f].replace(/\s/g, ''))) {
    ...
    // Tempat Lahir
    if (!d.tempatLahir || d.tempatLahir.length < 3 || !nameRegex.test(d.tempatLahir)) {
    ...
    if (!d.tanggalLahir) {
      ...
      if (birthDate >= today || age < 10 || age > 16) {
        newErrors.tanggalLahir = "Tanggal lahir tidak sesuai untuk pendaftaran SMP.";
      }
    }
    ...
    // H. Nama Wali
    if (!d.namaWali || d.namaWali.length < 3 || d.namaWali.length > 100 || !nameRegex.test(d.namaWali) || repeatRegex.test(d.namaWali.replace(/\s/g, ''))) {
    ...
    if (Object.keys(newErrors).length > 0) {
      setToastConfig({ show: true, message: "Harap perbaiki error pada formulir.", type: "error" });
      setTimeout(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(newErrors).length > 0) {
      setToastConfig({ show: true, message: "Periksa kembali kata sandi Anda.", type: "error" });
      setTimeout(() => {
        const firstErrorEl = document.querySelector('.error-input');
        if (firstErrorEl) firstErrorEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
      }, 100);
      ...
      <div className="w-full max-w-4xl">
        ...
        {gelombangLoading ? (
          ...
          <>
            ...
            <div className="flex items-center mb-6 relative">
              ...
              <div className="flex flex-col items-center relative">
                ...
                <div className="absolute left-full w-40 h-[2px] top-1/2 -translate-y-1/2 bg-gray-400">
                  <div
                    className={`h-full bg-blue-800 transition-all duration-500 ${step === 1 ? "w-0" : "w-full"
                      }`}
                  ></div>
                </div>
              </div>
            ...
            {/* Form Step 1 */}
            {step === 1 && (
              <form className="space-y-6 mx-0 md:mx-5 lg:mx-10" onSubmit={handleNext} noValidate>
                <div className="space-y-4 bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
                  ...
                  <div>
                    ...
                    />
                    {errors.namaLengkap && <p className="text-red-500 text-xs mt-1 font-medium">{errors.namaLengkap}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    />
                    {errors.asalSekolah && <p className="text-red-500 text-xs mt-1 font-medium">{errors.asalSekolah}</p>}
                  </div>

                  {/* Alamat Domisili */}
                  <h3 className="font-bold text-gray-800 mt-8 mb-2 border-b pb-2">Alamat Domisili</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      />
                      {errors.provinsi && <p className="text-red-500 text-xs mt-1 font-medium">{errors.provinsi}</p>}
                    </div>
                    <div>
                      <label className="block mb-1 font-semibold text-gray-700 text-sm">
                      ...
                      />
                      {errors.kotaKabupaten && <p className="text-red-500 text-xs mt-1 font-medium">{errors.kotaKabupaten}</p>}
                    </div>
                    <div>
                      <label className="block mb-1 font-semibold text-gray-700 text-sm">
                      ...
                      />
                      {errors.kecamatan && <p className="text-red-500 text-xs mt-1 font-medium">{errors.kecamatan}</p>}
                    </div>
                    <div>
                      <label className="block mb-1 font-semibold text-gray-700 text-sm">
                      ...
                      />
                      {errors.kelurahan && <p className="text-red-500 text-xs mt-1 font-medium">{errors.kelurahan}</p>}
                    </div>
                    <div>
                      <label className="block mb-1 font-semibold text-gray-700 text-sm">
                      ...
                      />
                      {errors.rtRw && <p className="text-red-500 text-xs mt-1 font-medium">{errors.rtRw}</p>}
                    </div>
                    <div>
                      <label className="block mb-1 font-semibold text-gray-700 text-sm">
                  ...
                  <h3 className="font-bold text-gray-800 mt-8 mb-2 border-b pb-2">Detail Personal</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      />
                      {errors.tempatLahir && <p className="text-red-500 text-xs mt-1 font-medium">{errors.tempatLahir}</p>}
                    </div>
                    ...
                    <div>
                      ...
                      />
                      {errors.tanggalLahir && <p className="text-red-500 text-xs mt-1 font-medium">{errors.tanggalLahir}</p>}
                    </div>
                    ...
                    <div>
                      ...
                      {errors.jenisKelamin && <p className="text-red-500 text-xs mt-1 font-medium">{errors.jenisKelamin}</p>}
                    </div>
                    ...
                    <div>
                      ...
                      />
                      {errors.noHp && <p className="text-red-500 text-xs mt-1 font-medium">{errors.noHp}</p>}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 mt-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <div>
                      ...
                      />
                      {errors.namaWali && <p className="text-red-500 text-xs mt-1 font-medium">{errors.namaWali}</p>}
                    </div>
                    <div>
                      <label className="block mb-1 font-semibold text-gray-700 text-sm">
                      ...
                      />
                      {errors.emailWali && <p className="text-red-500 text-xs mt-1 font-medium">{errors.emailWali}</p>}
                    </div>
                  </div>
                </div>
            ...
            {/* Form Step 2 */}
            {step === 2 && (
              <form className="space-y-6 mx-0 md:mx-5 lg:mx-10 bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100" onSubmit={handleSubmit} noValidate>
                {/* Judul */}
                <div className="border-b pb-4 mb-4">
                  <h2 className="text-xl font-bold text-gray-800">Buat Kata Sandi Akun</h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center mx-4 animate-in fade-in zoom-in duration-300">
            ...
            <p className="text-gray-500 mb-8 leading-relaxed">
              Akun Anda telah berhasil dibuat. Silakan login menggunakan <b>NISN</b> dan <b>Kata Sandi</b> yang telah Anda tentukan.
            </p>
            <button
