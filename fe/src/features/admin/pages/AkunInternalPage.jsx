...
export default function AkunInternalPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (modalMode === "add" || (modalMode === "edit" && formData.password)) {
      ...
      if (!passwordRegex.test(formData.password)) {
        setToastConfig({ show: true, message: "Kata sandi minimal 6 karakter dan harus mengandung huruf kecil, huruf besar, angka, dan simbol.", type: "error" });
        return;
      }
    }
  ...
  const paginatedVerifiers = filteredVerifiers.slice((pageVerifier - 1) * ITEMS_PER_PAGE, pageVerifier * ITEMS_PER_PAGE);
  ...
  const paginatedKepsek = filteredKepsek.slice((pageKepsek - 1) * ITEMS_PER_PAGE, pageKepsek * ITEMS_PER_PAGE);
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 md:p-8">
          <h2 className="text-xl font-bold text-gray-800 mb-6">Akun Verifikator</h2>
          <div className="flex justify-between items-center gap-4 mb-6">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-white border border-[#253b80] text-[#253b80] hover:bg-blue-50 px-4 py-2 rounded text-sm font-semibold transition-colors cursor-pointer shrink-0"
            >
                    ...
                    >
                      ...
                      <td className="p-4">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            onClick={() => handleOpenEdit("verifikator", item)}
                            title="Edit Akun"
                          ...
                          <button
                            onClick={() => handleToggleStatus("verifikator", item)}
                            title={item.status_aktif ? "Nonaktifkan Akun" : "Aktifkan Akun"}
                            className={`p-1.5 border rounded transition-colors cursor-pointer ${item.status_aktif
                          ...
                          <button
                            onClick={() => handleDelete("verifikator", item)}
                            disabled={item.status_aktif}
                            title="Hapus Akun"
                            className={`p-1.5 border rounded transition-colors ${item.status_aktif
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 md:p-8">
          <h2 className="text-xl font-bold text-gray-800 mb-6">Akun Kepala Sekolah</h2>
          <div className="flex justify-between items-center gap-4 mb-6">
            <div className="relative w-full md:w-80">
              <Search
                size={16}
                  ...
                  paginatedKepsek.map((item) => (
                    ...
                    >
                      ...
                      <td className="p-4">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            onClick={() => handleOpenEdit("kepsek", item)}
                            title="Edit Akun"
                            className="p-1.5 border border-gray-300 rounded text-gray-500 hover:bg-blue-50 hover:text-blue-600 transition-colors hover:border-blue-300 cursor-pointer"
                          >
                            <Edit2 size={16} />
                          ...
                          <button
                            onClick={() => handleToggleStatus("kepsek", item)}
                            title={item.status_aktif ? "Nonaktifkan Akun" : "Aktifkan Akun"}
                            className={`p-1.5 border rounded transition-colors cursor-pointer ${item.status_aktif
                              ? "border-orange-200 text-orange-500 hover:bg-orange-50 hover:text-orange-700"
                              : "border-green-200 text-green-500 hover:bg-green-50 hover:text-green-700"
                          ...
                          <button
                            onClick={() => handleDelete("kepsek", item)}
                            disabled={item.status_aktif}
                            title="Hapus Akun"
                            className={`p-1.5 border rounded transition-colors ${item.status_aktif
                              ? "border-gray-200 text-gray-300 opacity-50 cursor-not-allowed"
