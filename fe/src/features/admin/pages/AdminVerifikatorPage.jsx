...
import { Search, Plus, Eye, Edit2, Power, PowerOff, UserCircle2 } from "lucide-react";
...
export default function AdminVerifikatorPage() {
  ...
  const handleToggleStatus = async (verifier) => {
    const newStatus = !verifier.status_aktif;
    if (!window.confirm(`Yakin ingin ${newStatus ? 'mengaktifkan' : 'menonaktifkan'} akun verifikator ini?`)) return;
    try {
      await updateStatusVerifikator(verifier.id_verifikator, newStatus);
      setToastConfig({ show: true, message: `Akun berhasil di${newStatus ? 'aktifkan' : 'nonaktifkan'}!`, type: "success" });
      fetchVerifiers(false);
    } catch (err) {
      setToastConfig({ show: true, message: "Gagal mengubah status verifikator", type: "error" });
    }
  };
            ...
            <table className="w-full text-left border-collapse min-w-max">
              <thead>
                <tr className="bg-[#e2e8f0] border-b border-gray-200 text-sm font-semibold text-gray-700">
                  <th className="p-4 w-1/2">Akun Verifikator</th>
