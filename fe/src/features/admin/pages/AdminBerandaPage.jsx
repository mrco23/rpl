  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        ...
        setCarouselCards(dashboardData?.card || []);
      } catch (err) {
        console.error("[AdminBerandaPage]", err);
      ...
      <div className="relative mb-8 group">
        ...
        >
          {loading ? (
            ...
            <div className="w-full text-center text-gray-500 py-6">Data statistik belum tersedia</div>
          ) : (
            carouselCards?.map((c) => {
                ...
                <div key={keyName} className="bg-white rounded-xl px-6 py-4 border border-gray-200 shadow-sm flex flex-col border-l-4 border-l-[#253b80] min-w-[280px] shrink-0 snap-start transition-all duration-300">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <p className="text-3xl font-semibold text-gray-800 mt-1">{val ?? 0}</p>
                    </div>
