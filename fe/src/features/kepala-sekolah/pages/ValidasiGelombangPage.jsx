...
import { getValidasiGelombang, setujuiValidasiGelombang } from "../services/kepalaSekolahService.js";
...
export default function ValidasiGelombangPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setToast({ show: true, message: "Gagal mengambil data validasi", type: "error" });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSetujui = async () => {
        ...
        try {
            setIsApproving(true);
            setToast({ show: true, message: "Menyetujui laporan...", type: "info" });
            await setujuiValidasiGelombang(id);
            setConfirmModal({ show: false, item: null });
            setToast({ show: true, message: "Laporan gelombang berhasil disetujui!", type: "success" });
            fetchData();
        } catch (err) {
            console.error("Gagal menyetujui laporan:", err);
            setToast({ show: true, message: "Gagal menyetujui laporan", type: "error" });
        } finally {
        ...
        <div className="max-w-7xl mx-auto pb-10 space-y-6">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    {data.length > 0 && (
                        <div className="bg-white/15 backdrop-blur-sm border border-white/20 rounded-xl px-5 py-3 text-center shrink-0">
                            ...
                            <p className="text-xs font-semibold opacity-80 uppercase tracking-wide mt-0.5">Menunggu Validasi</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            {data.length === 0 ? (
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
                    ...
                    <h3 className="text-gray-800 font-bold text-lg mb-1">Semua Bersih!</h3>
                    <p className="text-gray-400 text-sm">Tidak ada laporan gelombang yang menunggu validasi saat ini.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                    {data.map((item) => {
                        const lulusPercent = item.totalPendaftar > 0
                            ? Math.round((item.totalLulus / item.totalPendaftar) * 100)
                            : 0;
                            ...
                            >
                                ...
                                <div className="p-5 border-b border-gray-100 flex items-start justify-between gap-3">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <h3 className="font-bold text-lg text-gray-900 leading-tight truncate">{item.nama}</h3>
                                        <div className="flex items-center gap-1.5 mt-1 text-sm text-gray-400">
                                            <CalendarDays size={13} />
                                            <span>{formatLongDate(item.tanggal_mulai)} – {formatLongDate(item.tanggal_selesai)}</span>
                                        </div>
                                    </div>
                                    {/* Kuota utilization ring area — simple text version */}
                                    <div className="text-right shrink-0">
                                        <p className="text-2xl font-black text-gray-900">{item.totalPendaftar}</p>
                                        <p className="text-xs text-gray-400 font-medium">dari {item.kuota} kuota</p>
                                    </div>
                                </div>
                                ...
                                <div className="px-5 pb-4">
                                    <div className="flex justify-between text-xs text-gray-500 mb-1.5 font-medium">
                                        ...
                                        <span className="font-bold text-gray-700">{lulusPercent}%</span>
                                    </div>
                                ...
                                <div className="px-5 pb-5 mt-auto">
                                    <button
                                        ...
                                        className="w-full bg-[#2f4aa0] text-white px-4 py-2 cursor-pointer rounded-md text-sm font-medium hover:bg-[#253b80] transition flex justify-center items-center gap-2"
                                    >
                ...
                <div className="space-y-4">
                    {confirmModal.item && (
                        <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
                            <p className="text-sm font-semibold text-blue-800">{confirmModal.item.nama}</p>
                            <p className="text-xs text-blue-600 mt-0.5">
                                {formatLongDate(confirmModal.item.tanggal_mulai)} – {formatLongDate(confirmModal.item.tanggal_selesai)}
                            </p>
                            <div className="flex gap-4 mt-3 text-xs text-blue-700">
                                <span><strong>{confirmModal.item.totalPendaftar}</strong> pendaftar</span>
                                <span><strong>{confirmModal.item.totalLulus}</strong> lulus</span>
                            </div>
                        </div>
                    )}
                    <p className="text-gray-600 text-sm">
                        Setelah disetujui, admin dapat melanjutkan proses penghapusan data gelombang ini. Tindakan ini <strong>tidak dapat dibatalkan</strong>.
                    </p>
