...
import { Users, FileCheck, UserX, UserCheck, UserCog, ClipboardCheck, BookOpen, UserCircle, Calendar } from "lucide-react";
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KpiCard title="Total Pendaftar" value={data.total_pendaftar} icon={Users} color="text-blue-600" bg="bg-blue-50" />
        ...
        <KpiCard title="Wawancara Ortu" value={data.wawancara_orang_tua} icon={UserCircle} color="text-purple-600" bg="bg-purple-50" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 lg:col-span-1 h-full">
          <h2 className="text-lg font-bold text-gray-800 mb-6">Rekap Status Pendaftaran</h2>
          <div className="space-y-5">
            <ProgressBar label="Menunggu Verifikasi" value={data.menunggu_verifikasi} total={data.total_pendaftar} color="bg-yellow-400" />
            <ProgressBar label="Perlu Perbaikan" value={data.perlu_perbaikan} total={data.total_pendaftar} color="bg-orange-400" />
            <ProgressBar label="Terverifikasi" value={data.terverifikasi} total={data.total_pendaftar} color="bg-teal-400" />
            <ProgressBar label="Wawancara Orang Tua" value={data.wawancara_orang_tua} total={data.total_pendaftar} color="bg-purple-400" />
            <ProgressBar label="Lulus" value={data.lulus} total={data.total_pendaftar} color="bg-green-500" />
            <ProgressBar label="Tidak Lulus" value={data.tidak_lulus} total={data.total_pendaftar} color="bg-red-500" />
          </div>
        </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-gray-200 text-sm text-gray-500">
                    <th className="pb-3 px-2 font-semibold">Gelombang</th>
                ...
                <tbody className="text-sm">
                  {data.gelombang.map((g, idx) => (
                    <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      <td className="py-3 px-2 font-medium text-gray-800">{g.nama}</td>
