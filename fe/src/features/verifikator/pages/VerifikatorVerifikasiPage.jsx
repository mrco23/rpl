...
import { STATUS_LABELS } from "../../../shared/constants/pendaftarStatus";
import { useSearchParams } from "react-router-dom";
import {
    resolveDocumentUrl,
    extractFileNameFromUrl,
    downloadDocumentFile,
} from "../../../shared/utils/documentHelper.js";
...
export default function VerifikatorVerifikasiPage() {
    ...
    const handleAssign = async (id) => {
        ...
        try {
            ...
            setToastConfig({
                show: true,
                message: err.message || "Gagal mengambil alih pemeriksaan",
                type: "error",
            });
        } finally {
    ...
    const handleVerifyAction = async (status) => {
        ...
        try {
            ...
            if (status === "terverifikasi") {
                setToastConfig({
                    show: true,
                    message: "Dokumen pendaftar berhasil diverifikasi!",
                    type: "success",
                });
            } else if (status === "perlu perbaikan") {
                setToastConfig({
                    show: true,
                    message: "Permintaan perbaikan berhasil dikirim ke pendaftar!",
                    type: "success",
                });
            }
        } catch (err) {
    ...
    const handleCancel = async () => {
        ...
        try {
            ...
            setToastConfig({
                show: true,
                message: err.message || "Gagal membatalkan pemeriksaan",
                type: "error",
            });
        } finally {
            setActionLoading(false);
                                ...
                                <div>
                                    ...
                                    <div className="space-y-4 bg-gray-50/50 p-5 rounded-2xl border border-gray-100">
                                        ...
                                        <DataRow
                                            label="Tanggal Lahir"
                                            value={formatDate(
                                                assignedApplicant.tanggal_lahir,
                                            )}
                                        />
                                        <DataRow
                                            label="Jenis Kelamin"
                                        ...
                                        {assignedApplicant.alamat ? (
                                            <div className="space-y-4">
                                                <DataRow
                                                    label="Provinsi"
                                                    value={
                                                        assignedApplicant.alamat.provinsi
                                                    }
                                                />
                                                <DataRow
                                                ...
                                                <DataRow
                                                    label="Kecamatan"
                                                    value={
                                                        assignedApplicant.alamat.kecamatan
                                                    }
                                                />
                                                <DataRow
                                                    label="Kelurahan"
                                                    value={
                                                        assignedApplicant.alamat.kelurahan
                                                    }
                                                />
                                                <DataRow
                                                    label="RT/RW"
                                                ...
                                                <DataRow
                                                    label="Kode Pos"
                                                    value={
                                                        assignedApplicant.alamat.kode_pos
                                                    }
                                                />
                                            </div>
                                        ) : (
            ...
            >
                <div className="flex flex-col items-center justify-center p-6 pb-2">
                    ...
                    <p className="text-center text-gray-600 text-sm mb-6 leading-relaxed">
                        Anda wajib mengisi <strong>Catatan Verifikasi</strong> untuk
                        memberi tahu pendaftar dokumen mana yang harus diperbaiki atau
                        diunggah ulang.
                    </p>
                    <button
...
function AssignedApplicantDetail({
    ...
    const isComplete = requiredDocs.every((reqDoc) =>
        assignedApplicant.dokumen?.some(
            (d) => d.nama_dokumen === reqDoc && d.jenis_dokumen,
        ),
    );
            ...
            <div className="p-6 lg:p-8 flex-1 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="flex gap-4">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="text-[11px] text-gray-500">
                                Mendaftar: {formatDate(assignedApplicant.tanggal_daftar)}
                            </span>
                        </div>
                    </div>
                ...
                >
                    ...
                    <p className="text-sm text-gray-600">
                        {assignedApplicant.status_pendaftaran === "menunggu verifikasi"
                            ? "Dokumen belum pernah diperiksa"
                            : "Pendaftar sedang memperbaiki dokumen"}
                    </p>
                </div>

                {assignedApplicant.dokumen.length < 4 && (
                    <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-8 flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-red-600 mt-1">
                                Pendaftar belum mengunggah seluruh 4 dokumen wajib
                                (Ijazah, Akte, KK, Pas Foto). Mohon beri catatan
                                perbaikan.
                            </p>
                        </div>
                    </div>
                            ...
                            >
                                <div className="flex flex-col min-w-0 mr-4">
                                    ...
                                    <span className="text-xs text-gray-500 mt-0.5 truncate">
                                        {extractFileNameFromUrl(doc.jenis_dokumen)}
                                    </span>
                                </div>
                                <div className="flex items-center gap-2 shrink-0">
                                    ...
                                    <button
                                        onClick={() =>
                                            downloadDocumentFile(
                                                doc.jenis_dokumen,
                                                `${assignedApplicant.nama_lengkap.replace(/\s+/g, "_")}_${doc.nama_dokumen.replace(/\s+/g, "_")}`,
                                            )
                                        }
                ...
                <div className="flex items-start gap-3 bg-white mb-8 mt-4 p-4 border border-blue-100 rounded-xl">
                    ...
                    <p className="text-sm text-blue-900 font-medium leading-snug">
                        Pastikan semua informasi dalam dokumen sesuai dengan data biodata.
                        <br className="hidden md:block" />
                        Isi catatan jika ada dokumen yang tidak sesuai atau tidak jelas.
                    </p>
                </div>
                ...
                <div className="mt-auto pt-4 border-t border-gray-100">
                    ...
                    <p className="text-[11px] text-center text-gray-400 mt-2">
                        Setelah melakukan tindakan, data pendaftar ini akan keluar dari
                        penanganan Anda.
                    </p>
                </div>
            </div>
