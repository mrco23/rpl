...
import { getLandingData } from "@services/siteContentService.js";

export default function LandingPage() {
  const [data, setData] = useState({ profile: null, achievements: [], news: [], extracurriculars: [] });

  useEffect(() => {
    getLandingData().then(setData).catch(() => {});
  }, []);

  const { profile, achievements, news } = data;
      ...
      <main>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          ...
          <div className="grid lg:grid-cols-2 gap-8 items-center bg-white rounded-[28px] overflow-hidden shadow-[0_20px_45px_var(--color-blue-normal)_/12%] border border-slate-100">
            <div className="h-full min-h-[360px] flex items-center justify-center bg-slate-50 p-6"><img src={profile?.foto_kepala_sekolah || "https://placehold.co/900x900?text=Kepala+Sekolah"} alt="kepala sekolah" className="w-[70%] h-[70%] object-cover rounded-[24px]" /></div>
            <div className="p-8 md:p-12">
              <div className="text-blue-normal font-bold text-xl mb-2">{profile?.nama_kepala_sekolah || "Kepala Sekolah"}</div>
              <div className="text-slate-500 mb-5">Kepala Sekolah</div>
              <p className="text-slate-600 leading-8">{profile?.sambutan_kepala_sekolah || profile?.deskripsi || "Informasi sambutan kepala sekolah akan tampil di sini."}</p>
            </div>
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-8">
              {news.slice(0, 3).map((item) => (
                <ImageCard key={item.id_berita} image={item.gambar} title={item.judul} description={item.deskripsi} extra={<div className="mt-5"><Link to={`/berita?id=${item.id_berita}`} className="text-blue-normal font-semibold">Baca selengkapnya</Link></div>} />
              ))}
            </div>
            <div className="mt-10 text-center">
