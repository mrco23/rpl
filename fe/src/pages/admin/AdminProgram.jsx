...
export default function AdminProgram() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fetchPrograms();
      setSuccessMessage(modalMode === 'add' ? 'Program unggulan berhasil ditambahkan!' : 'Program unggulan berhasil diperbarui!');
      setShowSuccessModal(true);
    } catch (err) {
      console.error(err);
      alert(`Gagal ${modalMode === 'add' ? 'menambah' : 'mengubah'} program unggulan`);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <>
          ...
          <table className="w-full text-left border-collapse min-w-max">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                   <td colSpan={3} className="p-8 text-center text-gray-500">Tidak ada program unggulan ditemukan</td>
                </tr>
              ) : (
                filteredPrograms.map((item) => (
                  <tr key={item.id_program} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
      ...
      <Modal 
        ...
        title={modalMode === 'add' ? 'Tambah Program Unggulan' : modalMode === 'edit' ? 'Edit Program Unggulan' : 'Detail Program Unggulan'}
      >
        {modalMode === 'detail' ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4 max-h-[80vh] overflow-y-auto pr-1">
            <div>
              ...
              <input 
                ...
                onChange={(e) => setFormData({...formData, nama_pu: e.target.value})}
            ...
            <div>
              ...
              <input 
                ...
                className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500" 
              />
              {modalMode === 'edit' && selectedItem?.gambar_pu && !formImage && (
