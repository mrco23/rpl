...
const STATUS_FILTER_MAP = {
    ...
    'Perlu Revisi': ['perlu perbaikan', 'unggah ulang'],
    'Diterima': 'lulus',
    'Wawancara': 'wawancara',
};
...
export default function AdminPendaftarPage() {
    ...
    const filteredData = data.filter(item => {
        ...
        const matchWave = selectedWave
            ? item.gelombang?.slug === selectedWave || item.gelombang?.nama?.toLowerCase().replace(' ', '-') === selectedWave
            : true;
        ...
        if (statusFilter !== null && statusFilter !== undefined) {
            if (Array.isArray(statusFilter)) {
                matchStatus = statusFilter.includes(item.status_pendaftaran);
            } else {
                matchStatus = item.status_pendaftaran === statusFilter;
            }
        }
    ...
    const getStatusBadge = (status) => {
        switch (status) {
            case 'menunggu verifikasi':
                return <span className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs font-semibold whitespace-nowrap">Menunggu Verifikasi</span>;
            case 'terverifikasi':
                return <span className="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs font-semibold whitespace-nowrap">Terverifikasi</span>;
            case 'perlu perbaikan':
            case 'unggah ulang':
                return <span className="px-2 py-1 bg-orange-100 text-orange-800 rounded-full text-xs font-semibold uppercase whitespace-nowrap">{status}</span>;
            case 'tidak lulus':
                return <span className="px-2 py-1 bg-red-100 text-red-800 rounded-full text-xs font-semibold whitespace-nowrap">Tidak Lulus</span>;
            case 'lulus':
                return <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-semibold whitespace-nowrap">Lulus</span>;
            default:
                return <span className="px-2 py-1 bg-gray-100 text-gray-800 rounded-full text-xs font-semibold uppercase whitespace-nowrap">{status?.replace('_', ' ')}</span>;
        }
    };
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="bg-white rounded-xl border border-[#d9e0ef] shadow-sm p-3 mb-6">
                    ...
                    <div className="flex flex-col gap-4">
                        {/* Baris 1: Dropdown Gelombang + Search */}
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                            ...
                            <select
                                ...
                                className="w-full sm:w-auto text-sm border border-gray-300 rounded px-2 py-1 focus:outline-none cursor-pointer"
                            >
                            ...
                            <button
                                ...
                                className={`w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-[#253b80] text-white text-sm font-medium px-4 py-2 rounded-md hover:bg-[#2646ae] transition-all ${
