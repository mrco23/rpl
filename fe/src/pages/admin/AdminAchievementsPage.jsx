...
export default function AdminAchievementsPage() {
  ...
  const [formData, setFormData] = useState({ judul_prestasi: '', deskripsi: '', peraih_prestasi: '' });
  ...
  return (
    <>
            ...
            <tbody>
              {loading ? (
                Array.from({ length: 5 }).map((_, i) => (
                 ...
                 <tr>
                   <td colSpan={3} className="p-8 text-center text-gray-500">Tidak ada prestasi ditemukan</td>
                 </tr>
              ) : (
                filteredAchievements.map((item) => (
                  <tr key={item.id_prestasi} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    <td className="p-4 text-sm text-gray-800 font-semibold">
                      <p className="max-w-[250px] truncate">{item.judul_prestasi}</p>
                      {item.peraih_prestasi && <p className="text-xs text-blue-600 font-normal mt-1 flex items-center gap-1">Peraih: {item.peraih_prestasi}</p>}
                    </td>
      ...
      <Modal 
        ...
        title={modalMode === 'add' ? 'Tambah Prestasi Baru' : modalMode === 'edit' ? 'Edit Prestasi' : 'Detail Prestasi'}
      >
        {modalMode === 'detail' ? (
          <div className="space-y-4 text-gray-800 max-h-[80vh] overflow-y-auto">
             {selectedItem?.gambar_prestasi && (
                <img src={selectedItem.gambar_prestasi} alt="Prestasi" className="w-full h-auto rounded-lg max-h-60 object-cover" />
             )}
             <div>
               <h3 className="font-bold text-lg">{selectedItem?.judul_prestasi}</h3>
               {selectedItem?.peraih_prestasi && <p className="text-sm font-semibold text-blue-700">Peraih: {selectedItem.peraih_prestasi}</p>}
             </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 max-h-[80vh] overflow-y-auto pr-1">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Judul Prestasi</label>
              <input 
                type="text" 
                required 
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Nama Peraih Prestasi</label>
              <input 
                type="text" 
                required 
                value={formData.peraih_prestasi}
                onChange={(e) => setFormData({...formData, peraih_prestasi: e.target.value})}
                className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500" 
              />
            </div>
            <div>
            ...
            <div>
              ...
              <input 
                ...
                className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500" 
              />
              {modalMode === 'edit' && selectedItem?.gambar_prestasi && !formImage && (
