...
## 🗄️ Primary Key `id_pendaftar`

Kolom `id_pendaftar` menggunakan `@default(autoincrement())` dari Prisma schema.
Database akan mengisi nilai ini secara otomatis saat insert — **tidak perlu mengirim `id_pendaftar` secara manual** dari layer aplikasi.
