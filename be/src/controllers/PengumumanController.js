...
export const getAll = async (req, res) => {
  try {
    ...
    res.json({ message: "success", data: data.map((item) => PengumumanService.serialize(req, item)) });
  } catch (error) {
...
export const create = async (req, res) => {
  try {
    ...
    res.status(201).json({ message: "Berhasil membuat Pengumuman", data: PengumumanService.serialize(req, created) });
  } catch (error) {
...
export const updateData = async (req, res) => {
  try {
    const updated = await PengumumanService.updatePengumumanData(req.user.id, req.params.id, req.body);
    res.json({ message: "Data berhasil diperbarui", data: PengumumanService.serialize(req, updated) });
  } catch (error) {
