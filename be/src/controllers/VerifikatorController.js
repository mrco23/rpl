...
class VerifikatorController {
	...
	assignPendaftar = async (req, res) => {
		try {
			...
			res
				.status(200)
				.json({ message: "Berhasil mengambil alih pemeriksaan pendaftar", data: updated });
		} catch (error) {
	...
	cancelVerifikasi = async (req, res) => {
		try {
			const { id } = req.params;
			const updated = await VerifikatorService.cancelVerifikasi(id, req.user.id);
			res.status(200).json({ message: "Proses pembatalan pendaftaran berhasil", data: updated });
		} catch (error) {
			res.status(400).json({ message: error.message });
		}
	};
