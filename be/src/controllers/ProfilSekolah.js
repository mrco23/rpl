...
class ProfilSekolah {
  addProfilSekolah = async (req, res) => {
    try {
      ...
      return res.status(201).json({ message: "success", data: ProfilSekolahService.serialize(req, request) });
    } catch (error) {
      if (error.code === "P2002") {
  ...
  updateProfilSekolah = async (req, res) => {
    try {
      ...
      return res.status(200).json({ message: "success", data: ProfilSekolahService.serialize(req, request) });
    } catch (error) {
  ...
  getProfilSekolah = async (req, res) => {
    try {
      ...
      return res.status(200).set("Cache-Control", "no-store").json({
        message: "success",
        data: ProfilSekolahService.serialize(req, profilSekolah),
      });
    } catch (error) {
  ...
  getPublicProfilSekolah = async (req, res) => {
    try {
      const profilSekolah = await ProfilSekolahService.getPublicProfil();
