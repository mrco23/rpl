export const validatePendaftarRegisterPayload = (payload) => {
    ...
    if (
        !d.nama_lengkap ||
        d.nama_lengkap.length < 3 ||
        d.nama_lengkap.length > 100 ||
        !nameRegex.test(d.nama_lengkap) ||
        repeatRegex.test(d.nama_lengkap.replace(/\s/g, ""))
    ) {
        isValid = false;
    ...
    if (!d.tanggal_lahir) {
        ...
        if (isNaN(birthDate.getTime()) || birthDate >= today || age <= 11) {
            isValid = false;
    ...
    if (
        !d.asal_sekolah ||
        /(SMA|SMK|MA\b|MAN\b|Paket C)/i.test(d.asal_sekolah) ||
        !/(SD|MI|Sekolah Dasar)/i.test(d.asal_sekolah)
    ) {
        isValid = false;
    ...
    if (
        !d.nama_wali ||
        d.nama_wali.length < 3 ||
        d.nama_wali.length > 100 ||
        !nameRegex.test(d.nama_wali) ||
        repeatRegex.test(d.nama_wali.replace(/\s/g, ""))
    ) {
        isValid = false;
    ...
    Object.keys(alamatFields).forEach((f) => {
        ...
        if (
            ...
            val.length < 3 ||
            !nameRegex.test(val) ||
            repeatRegex.test(val.replace(/\s/g, ""))
        ) {
            isValid = false;
    ...
    if (
        !d.kata_sandi ||
        !/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z\d]).{6,}$/.test(d.kata_sandi)
    ) {
        isValid = false;
        errors.kata_sandi =
